...
/*   Updated: 2023/09/06 13:07:22 by alsaeed          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_argc_and_ber(int argc, char *argv)
{
	...
	if (argc != 2)
	{
		write(2, "Error: The given arguments are not two!!\n", 42);
		exit (1);
	}
	len = ft_strlen(argv);
	...
	{
		write(2, "Error: The given map file is not a valid .ber file!!\n", 54);
		exit (1);
	}
}
...
void	parse_fd(int fd)
{
	...
	if (fd == -1)
	{
		write(2, "Error: The given map file is inaccessible!!\n", 45);
		exit (1);
	}
	if (!(read(fd, buf, 1)))
	{
		write(2, "Error: The given map file is empty!!\n", 38);
		exit (1);
	}
	if (close(fd) == -1)
	{
		write(2, "Error: The given map file has a close failure!!\n", 49);
		exit (1);
	}
}
